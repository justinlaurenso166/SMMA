...
const props = defineProps(["data_sensor","filter", "type"]);
...
if(props.type === "Daily"){
  ...
  x_label.value = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]
  for(let i = 0; i <= 6; i++){
      x_label.value.push(i);
      data_percepatan.value.push(0);
      data_kecepatan.value.push(0);
  ...
  props.data_sensor.forEach((e)=>{
    console.log(new Date(e.timestamps).toLocaleString('en-us', {weekday:'long'}))
    var idx = x_label.value.indexOf(new Date(e.timestamps).toLocaleString('en-us', {weekday:'long'}));
    console.log(idx)
    if(~idx){
      data_percepatan.value[idx] = e.percepatan
      data_kecepatan.value[idx] = e.kecepatan
  ...
  for(let i = 1; i <= 31; i++){
    ...
    props.data_sensor.forEach((e)=>{
    console.log(new Date(e.timestamps).getDate())
    var idx = x_label.value.indexOf(new Date(e.timestamps).getDate());
